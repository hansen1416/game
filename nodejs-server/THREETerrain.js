/**
 * THREETerrain.js 2.0.0-20220705
 *
 ...
 *
 * Usage: `var terrainScene = THREETerrain();`
 *
 ...
 *     as those for {@link THREETerrain.DiamondSquare}: an array of
 ...
 *     parameter for the `THREETerrain` function.
 ...
 *     include `THREETerrain.Linear` (the default), `THREETerrain.EaseIn`,
 *     `THREETerrain.EaseOut`, `THREETerrain.EaseInOut`,
 *     `THREETerrain.InEaseOut`, and any custom function that accepts a float
 ...
 *     `options.ySegments + 1`.
 */
const THREETerrain = function (options) {
	var defaultOptions = {
		after: null,
		...
		maxHeight: 100,
		minHeight: -100,
		optimization: THREETerrain.NONE,
		frequency: 2.5,
		steps: 1,
		stretch: true,
		turbulent: false,
	...
	var zs = THREETerrain.toArray1D(mesh.geometry.attributes.position.array);
	// if (
	// 	options.heightmap instanceof HTMLCanvasElement ||
	// 	options.heightmap instanceof Image
	// ) {
	// 	THREETerrain.fromHeightmap(zs, options);
	// } else 
 ...
 *   displayed. Valid options are the same as for {@link THREETerrain}().
 */
THREETerrain.Normalize = function (mesh, options) {
	var zs = THREETerrain.toArray1D(mesh.geometry.attributes.position.array);
	if (options.turbulent) {
		THREETerrain.Turbulence(zs, options);
	}
	if (options.steps > 1) {
		THREETerrain.Step(zs, options.steps);
		THREETerrain.Smooth(zs, options);
	}

	// Keep the terrain within the allotted height range if necessary, and do easing.
	THREETerrain.Clamp(zs, options);
 ...
 *
 *          - `THREETerrain.NONE`: Don't apply any optimizations. This is the
 *            default.
 *          - `THREETerrain.GEOMIPMAP`: Divide the terrain into evenly-sized
 *            sections with multiple levels of detail. For each section,
 *            display a level of detail dependent on how close the camera is.
 *          - `THREETerrain.GEOCLIPMAP`: Divide the terrain into donut-shaped
 ...
 */
THREETerrain.NONE = 0;
THREETerrain.GEOMIPMAP = 1;
THREETerrain.GEOCLIPMAP = 2;
THREETerrain.POLYGONREDUCTION = 3;
 ...
 */
THREETerrain.toArray2D = function (vertices, options) {
 ...
 */
THREETerrain.fromArray2D = function (vertices, src) {
 ...
 *
 * This is like {@link THREETerrain.toHeightmap} except that instead of
 ...
 *   {@link THREETerrain} function.
 * @param {Number} options
 *   The same as the options parameter for the {@link THREETerrain} function.
 */
THREETerrain.heightmapArray = function (method, options) {
	...
	method(arr, options);
	THREETerrain.Clamp(arr, options);
	return arr;
};
 ...
 */
THREETerrain.Linear = function (x) {
	return x;
};

// x = [0, 1], x^2
THREETerrain.EaseIn = function (x) {
	return x * x;
};

// x = [0, 1], -x(x-2)
THREETerrain.EaseOut = function (x) {
	return -x * (x - 2);
};
...
THREETerrain.EaseInOut = function (x) {
	return x * x * (3 - 2 * x);
};

// x = [0, 1], 0.5*(2x-1)^3+0.5
THREETerrain.InEaseOut = function (x) {
	var y = 2 * x - 1;
	return 0.5 * y * y * y + 0.5;
};

// x = [0, 1], x^1.55
THREETerrain.EaseInWeak = function (x) {
	return Math.pow(x, 1.55);
};

// x = [0, 1], x^7
THREETerrain.EaseInStrong = function (x) {
 ...
 *   of {@link THREETerrain}().
 */
THREETerrain.fromHeightmap = function (g, options) {
 ...
 *
 * Parameters are the same as for {@link THREETerrain.fromHeightmap} except
 ...
 */
THREETerrain.toHeightmap = function (g, options) {
 ...
 *   A map of settings that control how the terrain is constructed and
 *   displayed. Valid values are the same as those for the `options` parameter
 *   of {@link THREETerrain}() but only `maxHeight`, `minHeight`, and `easing`
 *   are used.
 */
THREETerrain.Clamp = function (g, options) {
	var min = Infinity,
		max = -Infinity,
		l = g.length,
		i;
	options.easing = options.easing || THREETerrain.Linear;
	for (i = 0; i < l; i++) {
		if (g[i] < min) min = g[i];
		if (g[i] > max) max = g[i];
	}
 ...
 *   by this operation.
 * @param {Number/Function} [e=THREETerrain.EaseInOut]
 ...
 *   Valid values include `THREETerrain.Linear`, `THREETerrain.EaseIn`,
 *   `THREETerrain.EaseOut`, `THREETerrain.EaseInOut`,
 *   `THREETerrain.InEaseOut`, and any custom function that accepts a float
 *   between 0 and 1 and returns a float between 0 and 1.
 ...
 */
THREETerrain.Edges = function (g, options, direction, distance, easing, edges) {
	var numXSegments =
		...
		k1,
		k2;
	easing = easing || THREETerrain.EaseInOut;
 ...
 *   between 0 and 1 and returns a float between 0 and 1.
 */
THREETerrain.RadialEdges = function (g, options, direction, distance, easing) {
 ...
 *   neighbors.
 */
THREETerrain.Smooth = function (g, options, weight) {
 ...
 *   of {@link THREETerrain}().
 */
THREETerrain.SmoothMedian = function (g, options) {
	var heightmap = new Float32Array(g.length),
		neighborValues = [],
		neighborKeys = [],
 ...
 */
THREETerrain.SmoothConservative = function (g, options, multiplier) {
	var heightmap = new Float32Array(g.length);
	for (
		var i = 0, xl = options.xSegments + 1, yl = options.ySegments + 1;
		i < xl;
 ...
 *   (g.length/2)^(1/4).
 */
THREETerrain.Step = function (g, levels) {
 ...
 *   The geometry's z-positions to modify with heightmap data.
 * @param {Object} [options]
 *   The same map of settings you'd pass to {@link THREETerrain()}. Only
 *   `minHeight` and `maxHeight` are used (and required) here.
 */
THREETerrain.Turbulence = function (g, options) {
 ...
 */
THREETerrain.MultiPass = function (g, options, passes) {
	var clonedOptions = {};
 ...
 */
THREETerrain.Curve = function (g, options, curve) {
 ...
 *
 * Parameters are the same as those for {@link THREETerrain.DiamondSquare}.
 */
THREETerrain.Cosine = function (g, options) {
 ...
 * Parameters are the same as those for {@link THREETerrain.DiamondSquare}.
 */
THREETerrain.CosineLayers = function (g, options) {
	THREETerrain.MultiPass(g, options, [
		{ method: THREETerrain.Cosine, frequency: 2.5 },
		{ method: THREETerrain.Cosine, amplitude: 0.1, frequency: 12 },
		{ method: THREETerrain.Cosine, amplitude: 0.05, frequency: 15 },
		{ method: THREETerrain.Cosine, amplitude: 0.025, frequency: 20 },
	]);
};
 ...
 *   of {@link THREETerrain}().
 */
THREETerrain.DiamondSquare = function (g, options) {
	...
	// THREETerrain.SmoothConservative(g, options);
};
 ...
 * Parameters are the same as those for {@link THREETerrain.DiamondSquare}.
 */
THREETerrain.Fault = function (g, options) {
	var d = Math.sqrt(
 ...
 *   displayed. Valid values are the same as those for the `options` parameter
 *   of {@link THREETerrain}().
 * @param {Function} [feature=THREETerrain.Influences.Hill]
 ...
 *   `THREETerrain.Influences` contains some useful functions for this
 *   purpose.
 ...
 */
THREETerrain.Hill = function (g, options, feature, shape) {
	...
	feature = feature || THREETerrain.Influences.Hill;
 ...
 *   purpose.
 */
THREETerrain.HillIsland = (function () {
	...
	return function (g, options, feature) {
		THREETerrain.Hill(g, options, feature, island);
	};
})();
...
(function () {
	 ...
	 * Parameters are the same as those for {@link THREETerrain.DiamondSquare}.
	 */
	THREETerrain.Particles = function (g, options) {
		var iterations =
				Math.sqrt(
					options.xSegments * options.xSegments +
						options.ySegments * options.ySegments
		...
		// THREETerrain.Smooth(g, options, 3);
	};
})();
 ...
 * Parameters are the same as those for {@link THREETerrain.DiamondSquare}.
 */
THREETerrain.Perlin = function (g, options) {
 ...
 * Parameters are the same as those for {@link THREETerrain.DiamondSquare}.
 */
THREETerrain.PerlinDiamond = function (g, options) {
	THREETerrain.MultiPass(g, options, [
		{ method: THREETerrain.Perlin },
		{ method: THREETerrain.DiamondSquare, amplitude: 0.75 },
		{
			method: function (g, o) {
				return THREETerrain.SmoothMedian(g, o);
			},
		},
	]);
};
 ...
 * Parameters are the same as those for {@link THREETerrain.DiamondSquare}.
 */
THREETerrain.PerlinLayers = function (g, options) {
	THREETerrain.MultiPass(g, options, [
		{ method: THREETerrain.Perlin, frequency: 1.25 },
		{ method: THREETerrain.Perlin, amplitude: 0.05, frequency: 2.5 },
		{ method: THREETerrain.Perlin, amplitude: 0.35, frequency: 5 },
		{ method: THREETerrain.Perlin, amplitude: 0.15, frequency: 10 },
	]);
};
 ...
 */
THREETerrain.Simplex = function (g, options) {
	noise.seed(Math.random());
	var range = (options.maxHeight - options.minHeight) * 0.5,
		divisor =
 ...
 * Parameters are the same as those for {@link THREETerrain.DiamondSquare}.
 */
THREETerrain.SimplexLayers = function (g, options) {
	THREETerrain.MultiPass(g, options, [
		{ method: THREETerrain.Simplex, frequency: 1.25 },
		{ method: THREETerrain.Simplex, amplitude: 0.5, frequency: 2.5 },
		{ method: THREETerrain.Simplex, amplitude: 0.25, frequency: 5 },
		{ method: THREETerrain.Simplex, amplitude: 0.125, frequency: 10 },
		{ method: THREETerrain.Simplex, amplitude: 0.0625, frequency: 20 },
	]);
};
...
(function () {
	 ...
	 * Parameters are the same as those for {@link THREETerrain.DiamondSquare}.
	 */
	THREETerrain.Value = function (g, options) {
		...
		// THREETerrain.Smooth(g, options, 1);
		THREETerrain.Clamp(g, {
			maxHeight: options.maxHeight,
			minHeight: options.minHeight,
			stretch: true,
 ...
 * Parameters are the same as those for {@link THREETerrain.DiamondSquare}.
 */
THREETerrain.Weierstrass = function (g, options) {
	var range = (options.maxHeight - options.minHeight) * 0.5,
 ...
 *    var material = THREETerrain.generateBlendedMaterial([
 ...
 *   `MeshLambertMaterial`.
 */
THREETerrain.generateBlendedMaterial = function (textures, material) {
 ...
 *     `THREETerrain()` or added to that return value; otherwise the position
 ...
 *   return value of a call to `THREETerrain()` or added to that return value;
 *   otherwise the position and rotation of the meshes will be wrong.
 */
THREETerrain.ScatterMeshes = function (geometry, options) {
	if (!options.mesh) {
		console.error(
			"options.mesh is required for THREETerrain.ScatterMeshes but was not passed"
		);
		return;
	}
	if (!options.scene) {
 ...
 *   `options.heightmap` parameter of the `THREETerrain` function).
 * @param {Object} options
 ...
 *   `THREETerrain` function). `options.minHeight` must equal `0` and
 ...
 *   `options.randomness` parameter to the {@link THREETerrain.ScatterMeshes}
 *   function.
 */
THREETerrain.ScatterHelper = function (method, options, skip, threshold) {
	skip = skip || 1;
	...
	var heightmap = THREETerrain.heightmapArray(method, clonedOptions);
 ...
 */
THREETerrain.Influences = {
	Mesa: function (x) {
		return 1.25 * Math.min(0.8, Math.exp(-(x * x)));
	},
	Hole: function (x) {
		return -THREETerrain.Influences.Mesa(x);
	},
	Hill: function (x) {
	...
	Valley: function (x) {
		return -THREETerrain.Influences.Hill(x);
	},
	Dome: function (x) {
		// Parabola
		return -(x + 1) * (x - 1);
 ...
 * @param {Number/Function} [e=THREETerrain.EaseIn]
 ...
 *   interpolating the height of each vertex along a curve. Valid values
 *   include `THREETerrain.Linear`, `THREETerrain.EaseIn`,
 *   `THREETerrain.EaseOut`, `THREETerrain.EaseInOut`,
 *   `THREETerrain.InEaseOut`, and any custom function that accepts a float
 ...
 */
THREETerrain.Influence = function (g, options, f, x, y, r, h, t, e) {
	f = f || THREETerrain.Influences.Hill; // feature shape
	...
	e = e || THREETerrain.EaseIn; // falloff
